 ...
 * #L%
 */
package fr.irit.smac.libs.tooling.examples.messaging;
...
public class AdvancedMessagingExample {
	...
	public static final void main(String[] args) {
		...
		for (int i = 0; i < nbAgents; i++) {
			...
			case 0: // generating a chatting to a group agent
				String groupId = null;
				if (groupIds.isEmpty()
						|| random.nextDouble() < groupCreationProb) {
		...
		System.out.println("Speed : " + (nbMillis) / ((double) nbReceivedMsgs)
				* 1000 + " μs / message");
	...
	private static class ChattingToAFriendAgent extends ChattingAgent {
		...
		public void sendMessages() {
			if (this.myFriend == null) {
				this.myFriend = this.msgBox.getDirectory().getAgentRef(
						this.myFriendId);
			}
	...
	private static class SometimeDisposingMsgBoxBroadcastingAgent extends
			BroadcastingAgent {
		...
		public SometimeDisposingMsgBoxBroadcastingAgent(String id,
				long randomSeed, double switchingProb) {
			super(id);
			this.enabled = true;
