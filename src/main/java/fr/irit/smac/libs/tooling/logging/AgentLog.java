...
public class AgentLog {
	...
	public static interface logItemDisplayData {
		 ...
		 * is greater than maxSize then the logger will truncate the remaining
		 * characters See truncatedSide to configure whether the characters are
		 ...
		 * actual string length to display
		 * 
		 * @param align
		 * @return
		 */
	...
	public static class Initializer {
		...
		private Initializer() {
			...
			this.cycleNumberDisplayData = new logItemDisplayDataImpl(
					"property{amasStepNumber}", true, 1, 3, null, Side.RIGHT,
					Side.RIGHT, "", "", this);

			this.logLevelDisplayData = new logItemDisplayDataImpl("level",
					true, 2, 5, null, Side.LEFT, Side.RIGHT, "", "", this);

			this.markerDisplayData = new logItemDisplayDataImpl("marker",
					false, 3, 6, null, Side.LEFT, Side.RIGHT, "", "", this);

			this.messageDisplayData = new logItemDisplayDataImpl("msg", true,
					4, null, null, Side.LEFT, Side.RIGHT, "", "", this);
		}
		 ...
		 * @return
		 */
		public Initializer clearLogFolderOnStartup(
				boolean clearLogFolderOnStartup) {
		 ...
		 * @return
		 */
		public Initializer customLogbackConfigurationFile(
				String customLogbackConfigurationFile) {
		 ...
		 */
		public logItemDisplayData stepNumberDisplay() {
		 ...
		 */
		public logItemDisplayData logLevelDisplay() {
		 ...
		 */
		public logItemDisplayData markerDisplay() {
		 ...
		 */
		public logItemDisplayData messageDisplay() {
			return this.messageDisplayData;
		}

		/**
		 * Method called in order to effectively initialize and configure the
		 * logging system
		 */
		public void initialize() {
			this.checkParameters();

			if (this.logPatternLayout == null
					|| this.logPatternLayout.isEmpty()) {
				this.logPatternLayout = this.computeLogPatternLayout();
			}

			if (this.clearLogFolderOnStartup && this.logEnabled
					&& this.logLevel != Level.OFF) {
				AgentLog.clearFolderContent(logFolderName);
			}

			if (this.customLogbackConfigurationFile != null
					&& !this.customLogbackConfigurationFile.isEmpty()) {
				AgentLog.initialiseLogback(this.customLogbackConfigurationFile);
			} else {
				AgentLog.initialiseLogback(this.logFolderName, this.logLevel,
						this.logPatternLayout);
			}

			AgentLog.configureLoggerContextProperties(this.logPatternLayout,
					this.logFolderName);
		}
			...
			if (this.customLogbackConfigurationFile != null) {
				...
				if (!f.exists() || !f.isFile() || !f.canRead()) {
					throw new IllegalArgumentException(
							"Invalid custom Logback configuration file !");
				}
			}
		...
		/**
		 * @return the log pattern layout computed from informations contained
		...
		private String computeLogPatternLayout() {
			...
			for (logItemDisplayDataImpl logItemData : this
					.getSortedLogItemDisplayData()) {
		...
		private List<logItemDisplayDataImpl> getSortedLogItemDisplayData() {
			List<logItemDisplayDataImpl> logItemDatas = Arrays
					.asList(new logItemDisplayDataImpl[] {
							this.cycleNumberDisplayData,
							this.markerDisplayData, this.logLevelDisplayData,
							this.messageDisplayData });

			Collections.sort(logItemDatas,
					new Comparator<logItemDisplayDataImpl>() {
						@Override
	...
	/**
	 * return initialize the logging system with the default configuration
	 */
	 ...
	 * @param logFolderName
	 */
	private static void configureLoggerContextProperties(
			final String logTemplate, final String logFolderName) {
		LoggerContext lc = (LoggerContext) LoggerFactory.getILoggerFactory();
	...
	/**
	 * Initialize logback just form a custom configuration file the following
	...
	private static void initialiseLogback(
			final String customLogbackConfigurationFile) {
		System.setProperty("logback.configurationFile",
				customLogbackConfigurationFile);
	}
	 ...
	 * @param logTemplate
	 */
	private static void initialiseLogback(final String logFolderName,
			final Level logLevel, final String logTemplate) {
		...
		appender.setAppenderFactory(new AgentFilesAppenderFactory(
				logFolderName, logTemplate));
		appender.start();
	...
	/**
	 * Factory used by the sifting appender used by default by the logging
	 * system
	 * 
	 * @author lemouzy
	 */
	private static class AgentFilesAppenderFactory implements
			AppenderFactory<ILoggingEvent> {

		private final String logFolderName;
		private final String logTemplate;

		private AgentFilesAppenderFactory(String logFolderName,
				String logTemplate) {
			this.logFolderName = logFolderName;
			this.logTemplate = logTemplate;
		}

		public Appender<ILoggingEvent> buildAppender(Context logcingContext,
				String fileName) throws JoranException {
			// first sets up the patterns for log layout
	...
	private static String escapePatternLayoutSpecialCharacters(
	...
	private static class logItemDisplayDataImpl implements logItemDisplayData {
		...
		private logItemDisplayDataImpl(String key, boolean displayed,
				int order, Integer minSize, Integer maxSize, Side align,
				Side truncatedSide, String prefix, String suffix,
		...
		private String generatePatternLayoutString() {
			return this.prefix
					+ "%"
					+ (this.minSize != null ? (this.align == Side.LEFT
							|| this.align == null ? "-" : "")
							+ this.minSize : "")
					+ (this.maxSize != null ? "."
							+ (this.truncatedSide == Side.RIGHT ? "-" : "")
							+ this.maxSize : "")
					+ this.key
					+ (this.key.contains("{") || this.key.contains("}") ? ""
							: "{}") + this.suffix;
		}
